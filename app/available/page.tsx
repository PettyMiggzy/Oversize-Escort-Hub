        ...
        <div style={{ color: "#fff", fontFamily: "sans-serif", background: "#0a0a0a", minHeight: "100vh", padding: 24 }}>
                <div style={{ maxWidth: 900, margin: "0 auto" }}>
                          <div style={{ marginBottom: 24 }}>
                                      <h1 style={{ fontSize: 24, fontWeight: 800, marginBottom: 4 }}>Escort Availability Board</h1>h1>
                                      <p style={{ color: "#9ca3af", fontSize: 13 }}>Pro escorts post availability. Pro carriers can send load alerts.</p>p>
                          </div>div>

                  {profile && ["pro", "member"].includes(profile.tier) && (
                    <div style={card}>
                                  <h2 style={{ fontSize: 16, fontWeight: 700, marginBottom: 16 }}>Post Your Availability</h2>h2>
                      {posted ? (
                                    <p style={{ color: "#22c55e" }}>Posted! Expires in 48 hours.</p>p>
                                  ) : (
                                    <div style={{ display: "flex", flexDirection: "column", gap: 10 }}>
                                                      <div style={{ display: "grid", gridTemplateColumns: "repeat(auto-fit,minmax(180px,1fr))", gap: 10 }}>
                                                                        ...
                                                                        <div>
                                                                                            <label style={{ fontSize: 11, color: "#9ca3af", display: "block", marginBottom: 4 }}>From *</label>label>
                                                                                            <input type="date" value={form.from} onChange={e => setForm(f => ({ ...f, from: e.target.value }))} style={inp} />
                                                                        </div>div>
                                                                        <div>
                                                                                            <label style={{ fontSize: 11, color: "#9ca3af", display: "block", marginBottom: 4 }}>To *</label>label>
                                                    ...
                                                    <button onClick={postAvailability} disabled={posting} style={btn()}>{posting ? "Posting..." : "Post Availability"}</button>button>
                                    </div>div>
                                )}
                    </div>div>
                        )}
                        ...
                        <div style={{ display: "flex", alignItems: "center", gap: 12, marginBottom: 20, flexWrap: "wrap" }}>
                                  <label style={{ fontSize: 13, color: "#9ca3af" }}>Filter:</label>label>
                                  <select value={filterState} onChange={e => setFilterState(e.target.value)} style={{ ...inp, width: "auto", minWidth: 120 }}>
                                              <option value="">All States</option>option>
                                    {US_STATES.map(s => <option key={s} value={s}>{s}</option>option>)}
                                  </select>select>
                                  <span style={{ fontSize: 12, color: "#9ca3af" }}>{filtered.length} escort{filtered.length !== 1 ? "s" : ""}</span>span>
                        </div>div>
                  ...
                  {Object.keys(byState).length === 0 ? (
                    ...
                    Object.entries(byState).sort(([a], [b]) => a.localeCompare(b)).map(([state, list]) => (
                                  <div key={state} style={card}>
                                                <h2 style={{ fontSize: 15, fontWeight: 700, marginBottom: 12, color: "#ff6600" }}>{state}</h2>h2>
                                                <div style={{ display: "flex", flexDirection: "column", gap: 10 }}>
                                                  {list.map(entry => (
                                                      <div key={entry.id} style={{ background: "#1a1a1a", borderRadius: 8, padding: 14, display: "flex", justifyContent: "space-between", alignItems: "flex-start", flexWrap: "wrap", gap: 10 }}>
                                                                          <div>
                                                                                                <div style={{ fontWeight: 700, fontSize: 14 }}>{entry.profiles?.full_name || "Escort"}</div>div>
                                                                                                <div style={{ fontSize: 12, color: "#9ca3af", marginTop: 2 }}>{entry.available_from} to {entry.available_to}</div>div>
                                                                            {entry.notes && <div style={{ fontSize: 12, color: "#d1d5db", marginTop: 4 }}>{entry.notes}</div>div>}
                                                        ...
                                                        {profile && (profile.tier === "pro" || profile.role === "carrier") && (
                                                                              <button onClick={() => sendLoadAlert(entry.escort_id)} style={btn("#3b82f6")}>Send Load Alert</button>button>
                                                                          )}
                                                      </div>div>
                                                    ))}
                                                </div>div>
