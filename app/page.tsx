  ...
  return (
    <div className="nav">
      ...
      <div className="nav-right" style={{ display: "flex", alignItems: "center", gap: 10, marginLeft: "auto" }}>
        {user && profile ? (
          <span className="nav-user"><button onClick={()=>setPage(profile?.role==='escort'?'dashboard-e':'dashboard-c')} style={{position:'relative',background:'none',border:'none',cursor:'pointer',padding:'4px',color:'var(--t1)',fontSize:20}} title="Notifications">{String.fromCodePoint(128276)}{unreadCount>0&&<span style={{position:'absolute',top:0,right:0,background:'#ff6600',color:'#fff',borderRadius:'50%',fontSize:9,width:16,height:16,display:'flex',alignItems:'center',justifyContent:'center',fontWeight:700}}>{unreadCount>9?'9+':unreadCount}</span>}</button>
...
// ─── SIGN IN PAGE function ConfirmSignupModal({onClose}:{onClose:()=>void}){return(<div style={{position:"fixed",inset:0,background:"rgba(0,0,0,0.85)",zIndex:9999,display:"flex",alignItems:"center",justifyContent:"center"}}><div style={{background:"#1a1a1a",border:"1px solid #ff6b00",borderRadius:12,padding:32,maxWidth:400,width:"90%",textAlign:"center"}}><div style={{fontSize:28,marginBottom:12}}>Almost there!</div><p style={{color:"var(--t2)",marginBottom:24,lineHeight:1.6}}>Check your email for a confirmation link. Click it to activate your account before signing in.</p><button className="btn" onClick={onClose} style={{background:"#ff6b00",color:"#000",padding:"12px 32px",fontSize:16,fontWeight:700,width:"100%",border:"none",borderRadius:8,cursor:"pointer"}}>Got it</button></div></div>);} ─────────────────────────────────────────────────────────────
...
function AdminPage({ setPage, user, profile }: any) {
  ...
  useEffect(() => { fetchBgcQueue(); fetchAdminStats(); fetchVerifyQueue(); fetchZoneSettings(); }, []) async function fetchZoneSettings(){supabase.from('sponsored_zone_settings').select('*').single().then(({data}:any)=>{if(data)setZoneSettings(data);}); supabase.from('profiles').select('id,full_name,email,sponsored_zones').eq('role','escort').not('sponsored_zones','is',null).then(({data}:any)=>setZoneSlots(data||[]));} async function saveZoneSettings(){setZoneSaving(true);await supabase.from('sponsored_zone_settings').upsert({id:1,...zoneSettings});setZoneSaving(false);alert('Zone settings saved!');}
