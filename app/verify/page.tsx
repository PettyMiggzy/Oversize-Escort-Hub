          ...
          <ul style={{ listStyle: "none", padding: 0, margin: "0 0 20px", display: "flex", flexDirection: "column", gap: 6 }}>
            {features.map((f) => (
                    <li key={f} style={{ fontSize: 13, color: "#d1d5db", display: "flex", alignItems: "center", gap: 8 }}>
                                <span style={{ color, fontSize: 14 }}>checkmark</span>span> {f}
                    </li>li>
                  ))}
          </ul>ul>
        )
  ...
  return (
        <div style={{ background: "#111", border: "1px solid #222", borderLeft: `4px solid ${t.badgeColor}`, borderRadius: 12, padding: "28px 24px", display: "flex", flexDirection: "column" }}>
                <div style={{ display: "flex", alignItems: "center", gap: 10, marginBottom: 16 }}>
                          <span style={{ background: t.badgeColor, color: "#fff", fontSize: 11, fontWeight: 800, letterSpacing: 1, padding: "3px 8px", borderRadius: 4 }}>{t.badge}</span>span>
                          <span style={{ fontSize: 18, fontWeight: 700 }}>{t.name}</span>span>
                </div>div>
                <p style={{ color: "#9ca3af", fontSize: 14, lineHeight: 1.6, marginBottom: 20, flex: 1 }}>{t.description}</p>p>
          ...
          {msg && <p style={{ fontSize: 13, color: status === "done" ? "#22c55e" : "#ef4444", marginTop: 10 }}>{msg}</p>p>}
        </div>div>
      )
}
  ...
  return (
        <div style={{ background: "#111", border: "1px solid #222", borderLeft: `4px solid ${t.badgeColor}`, borderRadius: 12, padding: "28px 24px", display: "flex", flexDirection: "column" }}>
                ...
                <div style={{ marginBottom: 16 }}>
                          <span style={{ fontSize: 28, fontWeight: 800, color: "#ff6600" }}>{t.price}</span>span>
                          <span style={{ fontSize: 13, color: "#9ca3af", marginLeft: 6 }}>{t.priceNote}</span>span>
                </div>div>
...
function DD214TierCard({ t }: { t: typeof TIERS[3] }) {
  ...
  return (
        <div style={{ background: "#111", border: "1px solid #222", borderLeft: `4px solid ${t.badgeColor}`, borderRadius: 12, padding: "28px 24px", display: "flex", flexDirection: "column" }}>
          ...
          {status === "done" ? (
                  <div style={{ color: "#22c55e", fontSize: 14, fontWeight: 600 }}>{msg}</div>div>
                ) : (
                  <>
                    ...
                    {msg && <p style={{ color: status === "error" ? "#ef4444" : "#22c55e", fontSize: 13, marginBottom: 8 }}>{msg}</p>p>}
                            <button onClick={handleSubmit} disabled={status === "uploading"} style={{ background: status === "uploading" ? "#555" : t.badgeColor, color: "#fff", border: "none", padding: "12px", borderRadius: 8, fontWeight: 700, fontSize: 14, cursor: status === "uploading" ? "not-allowed" : "pointer" }}>
          ...
          <main style={{ minHeight: "100vh", background: "#0d0d0d", color: "#fff", fontFamily: "sans-serif" }}>
                <div style={{ background: "linear-gradient(135deg, #111 0%, #1a0a00 100%)", padding: "60px 24px 48px", textAlign: "center", borderBottom: "1px solid rgba(255,102,0,0.2)" }}>
                        <div style={{ maxWidth: 700, margin: "0 auto" }}>
                                  <p style={{ color: "#ff6600", fontSize: 13, fontWeight: 700, letterSpacing: 2, textTransform: "uppercase", marginBottom: 12 }}>OEH VERIFICATION</p>p>
